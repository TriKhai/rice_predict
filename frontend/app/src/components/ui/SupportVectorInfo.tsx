...
const SupportVectorInfo: React.FC<SupportVectorInfoProps> = ({
  ...
  return (
    <div className="max-w-6xl mx-auto mt-5 grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="p-6 rounded-2xl bg-[#f7f7f8] dark:bg-[#0F1727] shadow border dark:border-gray-600 space-y-4 transition-colors duration-300 text-black dark:text-[#ececf1]">
        ...
        <div className="space-y-2">
          ...
          <div className="w-full bg-gray-200 dark:bg-gray-700 h-2 rounded">
            <div
              className="bg-[#7DD3FC] h-2 rounded"
              style={{ width: `${100}%` }}
            />
          </div>
      ...
      <div className="p-6 rounded-2xl bg-[#f7f7f8] dark:bg-[#0F1727] shadow border dark:border-gray-600 transition-colors duration-300 text-black dark:text-[#ececf1]">
        <h3 className="text-xl font-semibold mb-3">Chỉ số vector hỗ trợ</h3>
        <div className="bg-white dark:bg-gray-800 rounded-md p-3 border dark:border-gray-600 text-sm leading-relaxed max-h-40 overflow-y-auto">
          {displayedIndices.join(", ")}
        </div>
        <button
          onClick={() => setExpanded(!expanded)}
          className="mt-2 text-sm underline text-teal-200 hover:text-teal-100"
        >
